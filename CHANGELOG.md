...
### 🐛 Corrections
- Fix appel bloquant dans `InventoryManagerJSView.get()` : `filepath.is_file()` et `filepath.read_bytes()` sont maintenant exécutés via `hass.async_add_executor_job()` pour éviter le détection de blocking call par HA (Python 3.14).

## [2.2.4] - 2026-05-19

### 🐛 Corrections
- Fix erreur `CustomElementRegistry: name already used` : tous les appels `customElements.define()` sont maintenant protégés par `customElements.get()` pour éviter une double-définition lors d'un rechargement du panneau.
