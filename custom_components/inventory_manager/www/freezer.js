class InventoryManagerFreezer extends HTMLElement {
  ...
  async _addProduct() {
    ...
    if (duplicates.length > 0) {
      // Produit(s) similaire(s) trouvé(s)
      const sameDateDupes = duplicates.filter(p => p.expiry_date === date);
      
      if (sameDateDupes.length > 0) {
        // Même date de péremption → proposer d'incrémenter la quantité
        ...
        if (choice) {
          // Incrémenter la quantité du produit existant
        ...
        // Si Annuler → on continue pour créer un nouveau produit
      } else {
        // Date de péremption différente → informer et proposer
  ...
  async _removeSelectedProducts() {
    ...
    if (errors > 0) {
      alert(`${ids.length - errors} produit(s) retiré(s), ${errors} erreur(s).`);
    }
    
    this._closeRemoveModal();
  }

  // Camera for remove modal (reuses same barcode detection logic)
  ...
  _stopRemoveCamera() {
    if (this._removeScanInterval) {
      clearInterval(this._removeScanInterval);
      this._removeScanInterval = null;
    }
    if (this._removeStream) {
      this._removeStream.getTracks().forEach(track => track.stop());
